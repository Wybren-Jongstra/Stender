...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	public $scope = "email, user_about_me, user_birthday, user_hometown, user_website, read_stream, publish_actions, read_friendlists, user_likes";
	...
	*/
	function getUserActivity( $stream )
	{
		...
		return $activities;
 		...
 		/**
	* load the user likes profile from the IDp api client
	*/
	function getUserInterests()
	{
		// request user profile from fb api
		try{ 
			$data = $this->api->api('/me?fields=likes'); 
		}
		catch( FacebookApiException $e ){
			throw new Exception( "User profile request failed! {$this->providerId} returned an error: $e", 6 );
		} 
		...
		# store the user profile.
		$this->user->profile->identifier    = (array_key_exists('id',$data))?$data['id']:"";
		$this->user->profile->likes      = (array_key_exists('likes',$data))?$data['likes']['data']:"";
		$this->user->profile->displayName   = (array_key_exists('name',$data))?$data['name']:"";
		$this->user->profile->firstName     = (array_key_exists('first_name',$data))?$data['first_name']:"";
